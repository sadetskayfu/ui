...
export const MenuList = (props: MenuListProps) => {
  ...
  const interactiveElementsRef = useRef<Array<HTMLButtonElement | HTMLLinkElement>>([]);
  ...
  useEffect(() => {
    ...
    const updateInteractiveElements = () => {
      const interactiveElements = currentMenu.querySelectorAll<HTMLButtonElement | HTMLLinkElement>(
        "a, button"
      );
  ...
  const findNextEnabledItem = useCallback((currentIndex: number, direction: 1 | -1) => {
    ...
    for(let i = 1; i <= currentInteractiveElements.length; i++) {
      ...
      nextIndex = (currentIndex + step + currentInteractiveElements.length) % currentInteractiveElements.length;
      const isDisabled = currentInteractiveElements[nextIndex].getAttribute('data-disabled') === 'true'
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    switch (event.key) {
      ...
      case 'ArrowUp':
        ...
        isVisible && interactiveElementsRef.current.length > 0 ? newIndex = findNextEnabledItem(activeIndex, direction) : onOpen()
        break;
      case 'Enter':
      case ' ':
        event.preventDefault();
        if (isVisible) {
          activeIndex <= -1 ? onClose() : interactiveElementsRef.current[activeIndex].click()
        } else {
          onOpen()
        }
        break;
  ...
  }, [activeIndex, isVisible, onClose, onOpen, findNextEnabledItem])
